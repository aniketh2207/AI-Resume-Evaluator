...
def extract_education(text: str) -> list:
    ...
    education_keywords = {"bachelor", "master", "phd", "b.tech", "m.tech", "b.s.", "b.a.", "degree", "university", "institute","graduate","graduates","college","diploma","secondary","senior secondary","high school","middle school","school","board","ssc","hsc", "campus", "b.e.", "b.e"}
    ...
    for sent in doc.sents:
        ...
        if any(kw in lower_sent for kw in education_keywords):
            if not any(ex_kw in lower_sent for ex_kw in exclusion_keywords):
...
def calculate_composite_score(cand_data: dict, jd_data: dict, weights_dict: dict, thresholds_dict: dict):
    ...
    if jd_skills:
        ...
        if cand_skills["learning"]:
            learning_vecs = embedder.encode(cand_skills["learning"], convert_to_tensor=True)
        ...
        total_score += skills_score * (weights_dict.get("skills", 0) / 100.0)

    # --- 2. Evaluate Experience & Education (Standard Logic) ---
    for section in ['experience', 'education']:
        ...
        if section == 'experience' and jd_items:
            if any("fresh" in jd.lower() for jd in jd_items):
                section_scores[section] = 100.0
                total_score += 100.0 * (weights_dict.get(section, 0) / 100.0)
                continue
        
        if section == 'education':
            cand_items = normalize_text(cand_items)
            print("education normalized:" , cand_items )
            jd_items = normalize_text(jd_items)
            print("education normalized:" , jd_items )
        ...
        if section == 'education':
            print("--- EDUCATION DEBUG ---")
            print("JD Items Normalized:", jd_items)
            print("Cand Items Normalized:", cand_items)
            print("Threshold:", section_thresh)
            for i in range(len(jd_items)):
                if len(cand_items) > 0:
                    max_sim = sim_matrix[i].max().item()
                    print(f"JD Item {i} Max Similarity: {max_sim}")
                    # Print all similarities for this JD item against candidate items
                    for j in range(len(cand_items)):
                        print(f"  vs Cand Item {j}: {sim_matrix[i][j].item()}")
            print("-----------------------")
        ...
        total_score += sub_score * (weights_dict.get(section, 0) / 100.0)
